...
class MangaController {
  static async manga(req, res, next) {
    try {
      let { type, search, sort } = req.query;
      ...
      if (type !== "" && typeof type !== "undefined") {
        config = `?type=${type}&limit=15`;
      }

      const { data } = await axios({
        // url: `https://api.myanimelist.net/v2/manga` + config,
        url: `https://api.jikan.moe/v4/manga` + config,
        method: `GET`,
        // headers: {
  ...
  static async removeFromFavorit(req, res, next) {
    try {
      ...
      res.status(200).json("Manga has been removed from list");
    } catch (error) {
      next(error);
    }
  }
  ...
  static async getMidtransToken(req, res, next) {
    try {
      ...
      let parameter = {
        transaction_details: {
          order_id:
            "ORDERID-" + Date.now() + Math.floor(Math.random() * 9000000),
          gross_amount: 10000,
        },
        credit_card: {
          secure: true,
